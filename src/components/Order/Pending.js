    ...
    if (billDetail.length > 0) {
        for (let i = 0; i < user.length; i++) {
            ...
            for (let j = 0; j < billDetail.length; j++) {
                if (billDetail[j]?.bill?.account?.username === user[i] && billDetail[j]?.bill?.status === "Chờ xác nhận") {
